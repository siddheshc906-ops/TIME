...
async def mark_task_complete(task_id: str, current_user=Depends(get_current_user)):
    try:
        ...
        result = await tasks_col.update_one({"_id": ObjectId(task_id), "user_id": user_id}, {"$set": {"completed": True, "status": "completed", "completed_at": now}})
        if result.modified_count == 0:
            raise HTTPException(status_code=404, detail="Task not found")
        await daily_plans.update_many({"user_id": user_id}, {"$pull": {"pending_tasks": {"task_id": task_id}}})

        # Gap 2 Fix: write basic record to task_history so AI Coach
        # and analyzer have data even when no feedback time is logged
        try:
            estimated = task.get("estimated_time", task.get("estimatedTime", 1.0)) or 1.0
            existing = await task_history.find_one({"task_id": task_id, "user_id": user_id})
            if not existing:
            ...
            logger.warning(f"Could not write task_history for planner completion: {hist_err}")
...
# ╔══════════════════════════════════════════════════════════════════════════════
# ║  FOCUS SESSIONS  (Gap 1 Fix)
# ║  Save focus sessions to DB so AI Coach and Performance page stay in sync
# ║  across devices and after localStorage clears.
# ╚══════════════════════════════════════════════════════════════════════════════

class FocusSessionSave(BaseModel):
...
async def save_focus_session(body: FocusSessionSave, current_user=Depends(get_current_user)):
    try:
        user_id = str(current_user["_id"])
        now     = datetime.now(timezone.utc)
        ...
        total_sessions = await db.focus_sessions.count_documents({"user_id": user_id})
        return {"success": True, "message": "Focus session saved", "total_sessions": total_sessions}
    except Exception as e:
...
async def get_focus_sessions(current_user=Depends(get_current_user)):
    try:
        ...
        logger.error(f"Get focus sessions error: {e}")
        return {"success": False, "sessions": [], "total_sessions": 0, "total_minutes": 0, "total_hours": 0}
...
class DayContextRequest(BaseModel):
    ...
    # College/work hours for "before/after college" AI understanding
    college_start: Optional[float] = None
    college_end:   Optional[float] = None
    college_label: Optional[str]   = None
...
async def save_day_context(request: DayContextRequest, current_user=Depends(get_current_user)):
    ...
    # ── CRITICAL FIX: clamp wake_up so it is always before college_start ──
    # If wake_up >= college_start, scheduling "before college" is impossible.
    # We clamp wake_up to (college_start - 1h) so there is always a valid window.
    c_start = ctx.get("college_start")
    wake    = ctx.get("wake_up")
    if c_start is not None and wake is not None and wake >= c_start:
        clamped_wake = max(0.0, c_start - 1.0)
        logger.warning(
            f"⚠️  save_day_context: wake_up ({wake}) >= college_start ({c_start}) for user "
            f"{user_id} — clamping wake_up to {clamped_wake}"
        )
        ctx["wake_up"] = clamped_wake

    ctx["user_id"]    = user_id
    ctx["has_custom"] = True
    ctx["updated_at"] = datetime.now(timezone.utc)

    def decimal_to_ampm(dec):
        if dec is None: return None
        h = int(dec); m = int((dec - h) * 60)
        period = "AM" if h < 12 else "PM"
        display = h % 12 or 12
        return f"{display}:{m:02d} {period}"

    # Store formatted strings for AI prompt use
    ctx["wake_time_str"]     = decimal_to_ampm(ctx["wake_up"])
    ctx["sleep_time_str"]    = decimal_to_ampm(request.day_end)
    ctx["college_start_str"] = decimal_to_ampm(request.college_start)
    ctx["college_end_str"]   = decimal_to_ampm(request.college_end)

    await db.user_day_context.update_one(
        {"user_id": user_id}, {"$set": ctx}, upsert=True
    )
    logger.info(f"✅ Routine saved for user {user_id}: wake={ctx['wake_time_str']}, college={ctx['college_start_str']}–{ctx['college_end_str']}")
    return {"success": True, "message": "Routine saved! AI will now schedule around your hours."}
